...
def setup_logging(log_dir: str) -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(os.path.join(log_dir, "train_standard_rl.log")),
        ],
    )
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("STANDARD RL AGENT TRAINING (NO LSTM FORECAST)")
    logger.info("=" * 60)
    logger.info(cfg)

    pipeline = RLTrainingPipeline(
        cfg=cfg,
        use_forecast=False,            # ← no LSTM forecast in state
    ...
    history_path = os.path.join(cfg.base.paths.results, "rl_standard_training_history.json")
    with open(history_path, "w") as f:
        json.dump(results["history"], f)
    logger.info(f"Training history saved to {history_path}")
